...
public class TabbedPaneUI extends BasicTabbedPaneUI {
  ...
  private Color outerHighlight = MyLookAndFeel.getControlDisabled();
  private Color innerHighlight = MyLookAndFeel.getPrimaryControlHighlight();
  private Color outerShadow = MyLookAndFeel.getControlDarkShadow();
  private Color innerShadow = MyLookAndFeel.getDesktopColor();
  ...
  protected void paintTabArea(Graphics g, int tabPlacement, int selectedIndex) {
    ...
    Insets s = getTabInsets(tabPlacement,selectedIndex);
    //this.getTabInsets(tabPlacement,selectedIndex);
    
    //System.out.println("l"+s.left+"r"+s.right+"t"+s.top+"b"+s.bottom);
    //System.out.println("com"+com.getWidth()+":"+com.getHeight());
    
    //g.fillRect(0,0,com.getWidth(),com.getHeight());
  }
  */
...
//          int xoffset = isSelected ? 0 : (tabIndex > 0) 0;
        ...
        {
          g.drawLine(x, y + 2, x, y + h - 1 + yoffset); // left highlight
        }
        g.drawLine(x + 1, y + 1, x + 1, y + 1); // top-left highlight
        g.drawLine(x + 2, y, x + w - 3, y); // top highlight

        g.setColor(innerHighlight);
        g.drawLine(x + 1, y + 2, x + 1, y + h - 1 + yoffset); // left highlight
